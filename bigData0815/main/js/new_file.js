...
var mapServerURL = "http://113.108.98.19:8888/SZRiverSys/";
...
function initialMap() {
	...
	StationLayer = new ol.layer.Vector({
		...
		style: function(feature) {
			...
			var tmpStyle = StationLayerStyles[0];
			if(!tmpStyle) {
...
function loadStationLayers() { //加载断面站点
	...
	$.ajax({
		url: mapServerURL + "ServiceHandler/StationHandler.ashx",
		type: "Post",
		data: data,
		dataType: "json",
		success: function(joResult) {
			if(joResult.success == true) {
				M_map.removeLayer(StationLayer);
				M_map.addLayer(StationLayer);

				var tmpObjs = joResult.msg;
				if(StationLayerVectorSource == null) {
					StationLayerVectorSource = new ol.source.Vector();
					StationLayer.setSource(StationLayerVectorSource);
				} else
					StationLayerVectorSource.clear(true);
...
riverSectionSource = new ol.source.Vector({        
	...
	url: mapServerURL + 'ServiceHandler/RiverBoundaryHandler.ashx?method=getorigriverboundary', //'riversection.geojson',
	//113.108.98.19:8888/SZRiverSys/ServiceHandler/RiverBoundaryHandler.ashx?method=getriversectionbytime&time=2019-03-24 20:42:00.000
